...
The reviewed fixes were revalidated through production commit `a1c28a3f` using the same isolated Windows ARM64 app state. UI Automation at the preceding UI-only production commit `a9c2916f` confirmed the Extensions page, Skills and Plugins tabs, Agent selector, installed inventory, ClawHub links, and the single older-Gateway compatibility message. The app remained responsive after live tab selection. Automated proof through `a1c28a3f` also passed:

- Full build on Windows ARM64.
- Shared: 3,960 passed, 32 environment-gated skips.
- Tray: 2,897 passed.
- Extensions page and view-model contracts: 31 passed.
- Native Axe scan for `ExtensionsPage`: 1 passed.
- Connection-epoch and Gateway extension contracts: 17 passed, including forced reconnects between review or consent validation and the final transport write for Skill install, Plugin install, and Plugin enable, plus preservation of the selected-agent refresh after a successful legacy skill toggle. In all three reconnect cases the operation failed closed before the test server received the forbidden mutation request.
